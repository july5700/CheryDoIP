...
import re
from dtc import EnhancedDTCParser
...
class MainWindow(QMainWindow, ui_AIBox.Ui_Dialog):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
    ...
    def application_service_init(self):
        # 10 服务
        self.pushButton_1001.clicked.connect(lambda: self.frame.positive_response('1001'))
        self.pushButton_1002.clicked.connect(lambda: self.frame.positive_response('1002'))
        self.pushButton_1003.clicked.connect(lambda: self.frame.positive_response('1003'))

        # 27 服务
        self.pushButton_SecurityAccess_0102.clicked.connect(self.frame.send_security_access_2701)
        self.pushButton_SecurityAccess_0506.clicked.connect(self.frame.send_security_access_2705)
        self.pushButton_SecurityAccess_191a.clicked.connect(self.frame.send_security_access_2719)

        # 11 服务
        self.pushButton_hardReset.clicked.connect(lambda: self.frame.positive_response('1101'))

        # 28 服务
        self.pushButton_2800.clicked.connect(lambda: self.frame.positive_response('2800'))
        self.pushButton_2801.clicked.connect(lambda: self.frame.positive_response('2801'))
        self.pushButton_2802.clicked.connect(lambda: self.frame.positive_response('2802'))
        self.pushButton_2803.clicked.connect(lambda: self.frame.positive_response('2803'))
        ...
        # 85 服务
        self.pushButton_8501.clicked.connect(lambda: self.frame.positive_response('8501'))
        self.pushButton_8502.clicked.connect(lambda: self.frame.positive_response('8502'))

        # 3E 服务
        self.pushButton_3e00.clicked.connect(self.toggle_send_3e)

        # 19 服务
        self.pushButton_1902.clicked.connect(lambda: self.frame.positive_response('1902'))
        self.pushButton_1903.clicked.connect(lambda: self.frame.positive_response('1903'))
        self.pushButton_1904.clicked.connect(lambda: self.frame.positive_response('1904'))
        self.pushButton_1906.clicked.connect(lambda: self.frame.positive_response('1906'))
        self.pushButton_190a.clicked.connect(lambda: self.frame.positive_response('190a'))
    ...
    def dtc_init(self):
        ...
        # self.pushButton_check_all_dtc.clicked.connect(self.check_all_dtc_aibox)
    ...
    def aibox_dtc_init(self):
        self.comboBox_1902.clear()
        self.comboBox_1902.addItems([
            "190200",
            "190201",
            "190202",
            "190203",
        ...
        # logger.info(f"going to send {self.comboBox_1902.currentText()}")
        self.comboBox_1902.currentTextChanged.connect(lambda: self.check_all_dtc_aibox(self.comboBox_1902.currentText()))
    ...
    def check_all_dtc_aibox(self, three_byte):
        # logger.info(f"going to send {three_byte}")
        res = self.frame.basic_send_response(three_byte)
        res_list = self.dtc.get_known_dtc_with_raw_status(res)

        if res_list:
            logger.info("🔍 检测到以下故障码及其原始状态码：")
            # logger.info(f"🔍 检测到：{res_list}")
            for desc in res_list:
                logger.info(f"  - {desc[0]} | 状态: {desc[1]} | {desc[2]}")
        else:
