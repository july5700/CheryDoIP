...
sec_key_2701 = conf.get('current.sec_key_2701')
sec_key_2711 = conf.get('current.sec_key_2711')
sec_key_2719 = conf.get('current.sec_key_2719')
pinp = conf.get('current.pinp_2705')
...
def cal_ace_emac_2711(seed):
    """
    :param seed:
    :return:
    """
    key = unhexlify(sec_key_2711)  # 20251114
    seed = unhexlify(seed)
    mac = CMAC.new(key, seed, ciphermod=AES)
    # print("AES_CMAC:", mac.hexdigest())
    return mac.hexdigest()
...
def cal_ace_emac_aibox_2701(seed):
    """
    给27 01/02用的加密方法，未来可能还要给到27 61/62
    :param seed:
    :return:
    """
    key = unhexlify(sec_key_2701)  # 20251114
    seed = unhexlify(seed)
    mac = CMAC.new(key, seed, ciphermod=AES)
    # print("AES_CMAC:", mac.hexdigest())
    return mac.hexdigest()
...
def cal_ace_emac_aibox_2719(seed):
    """
    给27 01/02用的加密方法，未来可能还要给到27 61/62
    :param seed:
    :return:
    """
    key = unhexlify(sec_key_2719)  # 20251114
    seed = unhexlify(seed)
    mac = CMAC.new(key, seed, ciphermod=AES)
    # print("AES_CMAC:", mac.hexdigest())
    return mac.hexdigest()
...
def cal_tea_variant_aibox(sd, pinp=pinp):
    """
    The seed-to-key algorithm used in 05/06 (based on the TEA variant)
