...
payload_len = conf.get("current.payload_len")
class Parser:
...
class DoIPClient:
    ...
    def __init__(
        ...
        if self._activation_type is not None:
            logger2.debug(f"Activate type: {hex(self._activation_type)}")
            result = self.request_activation(self._activation_type, disable_retry=True)
            # # logger.critical(f"activatee result = {result} type = {type(result)}")
            # logger.info(f"activatee result.response_code = {result.response_code} type = {type(result.response_code)}")
    ...
    def _pack_doip(protocol_version, payload_type, payload_data):
        if payload_len is None:
            len_payload = len(payload_data)
        else:
            len_payload = int(payload_len)
        if payload_type == 5:
            len_payload = 7
